...
spk_test <- data.frame(
  var = ("walk"),
  sparkline = spk_chr(arg_data$apple_mtr_walking)
)

arg_data_test <- arg_data_clean %>%
  summarise(walk = mean(apple_mtr_walking, na.rm = TRUE))


joint <- bind_cols(arg_data_test, spk_test)
